...
class IoTPowerPipeline:
    ...
    def run_forecasting(self, enhanced=True, hyperparameter_tuning=False):
        """Run forecasting pipeline with enhanced models"""
        self.logger.info(f"Running {'enhanced' if enhanced else 'basic'} forecasting pipeline...")
        
        try:
            if enhanced:
                # Use enhanced forecasting with multiple models
                results, forecast_df = self.forecasting_layer.run_enhanced_forecasting_pipeline(
                    run_hyperparameter_tuning=hyperparameter_tuning
                )
                self.logger.info("Enhanced forecasting completed successfully")
            else:
                # Use basic forecasting
                results, forecast_df = self.forecasting_layer.run_forecasting_pipeline(enhanced=False)
                self.logger.info("Basic forecasting completed successfully")
...
def main():
    ...
    parser.add_argument("--enhanced", action="store_true",
                       help="Use enhanced forecasting with multiple models")
    parser.add_argument("--hyperparameter-tuning", action="store_true",
                       help="Enable hyperparameter tuning (slower but more accurate)")
